...
def get_market_news(query=None, limit=5):
    ...
    """
    try:
        # Import the fetch_yahoo_finance_news function from stock_fetcher.py
        import sys
        import os
        # Add the parent directory to sys.path to allow importing stock_fetcher
        parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        if parent_dir not in sys.path:
            sys.path.append(parent_dir)
        from stock_fetcher import fetch_yahoo_finance_news
        
        # Use the dedicated function to fetch real news
...
def generate_default_news(limit=5):
    ...
    for i in range(limit):
        ...
        title = f"Markets react to latest developments in {topic}"
